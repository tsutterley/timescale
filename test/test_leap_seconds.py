...
def test_leap_seconds():
    ...
    test_gps_leaps = timescale.time.get_leap_seconds()
    assert all((v==t) for v,t in zip(valid_gps_leaps,test_gps_leaps))

# PURPOSE: update leap second file
def test_update_leap_seconds():
    timescale.time.update_leap_seconds(verbose=False, mode=0o775)
    FILE = timescale.utilities.get_data_path(['data','leap-seconds.list'])
    assert FILE.exists()
